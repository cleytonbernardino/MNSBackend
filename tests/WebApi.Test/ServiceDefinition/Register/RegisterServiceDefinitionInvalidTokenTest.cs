...
public class RegisterServiceDefinitionInvalidTokenTest(CustomWebApplicationFactory factory) : MmsClassFixture(factory)
{
    ...
    public async Task Error_Invalid_Token(string culture)
    {
        ...
        var response = await DoPostAsync(request, token: "TokenInvalid", culture);
    ...
    public async Task Error_Without_Token(string culture)
    {
        ...
        var response = await DoPostAsync(request, token: "", culture: culture);
        response.StatusCode.ShouldBe(HttpStatusCode.Unauthorized);
    }
